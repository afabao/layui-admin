layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#meterial-data'
            , height: 415
            , url: $tool.getContext() + 'meterial/meterialList.do' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                ...
                , {field: 'goodsDescribe', title: '货品描述'/*, width: '10%'*/}
        ...
        //为toolbar添加事件响应
        table.on('tool(meterialFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    //添加供应商
    $(".meterialsAdd_btn").click(function () {
        var index = layui.layer.open({
            title: "添加供应商",
            type: 2,
            content: "addMeterial.html",
    ...
    function delMeterial(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.DeleteMeterial(req,function (data) {
    ...
    function editMeterial(id){
        var index = layui.layer.open({
            title: "编辑供应商",
            type: 2,
            content: "editMeterial.html?id="+id,
            success: function (layero, index) {
                setTimeout(function () {
